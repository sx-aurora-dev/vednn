...
#pragma clang loop unroll(full)
  ...
  for (int64_t ow = 0 ; ow < outWidth; ow+=VLEN) {

    const int64_t vl = outWidth - ow < VLEN ? outWidth - ow : VLEN ;

    __vr vrseq = _vel_vseq_vl(vl) ;			// x
...
#pragma clang loop unroll(full)
    ...
    __vr vrx   = _vel_vaddsl_vsvl(ow, vrseq, vl) ;
    ...
    for (int64_t r = 2-oh ; r < kernHeight; r++) {
      for (int64_t c = 0; c < inChannelGroup; c++) {
	const float *pInChannel = pIn + inGroupOffset + ((n * inChannel + c) * inHeight * inWidth ) ;

	/* memory access errors might be caused */
	__vr vrin_s0 = _vel_vldu_vssl(4,&pInChannel[ow+(oh+r-2)*inWidth-2], vl) ;
	__vr vrin_s1 = _vel_vldu_vssl(4,&pInChannel[ow+(oh+r-2)*inWidth-1], vl) ;
	__vr vrin_s2 = _vel_vldu_vssl(4,&pInChannel[ow+(oh+r-2)*inWidth  ], vl) ;
	__vr vrin_s3 = _vel_vldu_vssl(4,&pInChannel[ow+(oh+r-2)*inWidth+1], vl) ;
	__vr vrin_s4 = _vel_vldu_vssl(4,&pInChannel[ow+(oh+r-2)*inWidth+2], vl) ;
...
#define VFMAD(VRIN, R, S) 										\
	...
	vrin_s0 = _vel_vmrg_vvvml(_vel_vbrds_vsl(0.0f, vl), vrin_s0, vmw_s0, vl) ;
	VFMAD(vrin_s0, r, 0) ;
	vrin_s1 = _vel_vmrg_vvvml(_vel_vbrds_vsl(0.0f, vl), vrin_s1, vmw_s1, vl) ;
	VFMAD(vrin_s1, r, 1) ;
	VFMAD(vrin_s2, r, 2) ;
	vrin_s3 = _vel_vmrg_vvvml(_vel_vbrds_vsl(0.0f, vl), vrin_s3, vmw_s3, vl) ;
	VFMAD(vrin_s3, r, 3) ;
	vrin_s4 = _vel_vmrg_vvvml(_vel_vbrds_vsl(0.0f, vl), vrin_s4, vmw_s4, vl) ;
	VFMAD(vrin_s4, r, 4) ;
#undef VFMAD
#undef FILTER_OFFSET
      } // inChannel
...
#pragma clang loop unroll(full)
  ...
  for (int64_t op = 2*outWidth ; op < (outHeight-2) * outWidth; op+=VLEN) {
...
#pragma clang loop unroll(full)
  ...
  for (int64_t oh = outHeight-2 ; oh < outHeight; oh++)
  for (int64_t ow = 0 ; ow < outWidth; ow+=VLEN) {
...
#pragma clang loop unroll(full)
    ...
    for (int64_t r = 0; r < kernHeight + (outHeight-oh) -3  ; r++) {
      for (int64_t c = 0; c < inChannelGroup; c++) {
	const float *pInChannel = pIn + inGroupOffset + ((n * inChannel + c) * inHeight * inWidth ) ;
