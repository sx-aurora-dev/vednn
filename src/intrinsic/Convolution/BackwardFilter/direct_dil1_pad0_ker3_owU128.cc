...
#pragma clang loop unroll(full)
	...
	const float *pInChannel = pIn + inGroupOffset + ( ((n * inChannel + c) * inHeight + y * strideHeight ) * inWidth ) ;

	const int64_t outIndex = gOutGroupOffset + ((n * gOutChannel + k) * gOutHeight + y) * gOutWidth ;


	__vr vrpin_r0s0 = _vel_vsfa_vvssl(vrij, 2, (uint64_t)(pInChannel+0*inWidth+0), vl) ;
	__vr vrin_r0s0 = _vel_vgtu_vvssl(vrpin_r0s0, 0, 0, vl) ;
	__vr vrpin_r0s1 = _vel_vsfa_vvssl(vrij, 2, (uint64_t)(pInChannel+0*inWidth+1), vl) ;
	__vr vrin_r0s1 = _vel_vgtu_vvssl(vrpin_r0s1, 0, 0, vl) ;
	__vr vrpin_r0s2 = _vel_vsfa_vvssl(vrij, 2, (uint64_t)(pInChannel+0*inWidth+2), vl) ;
	__vr vrin_r0s2 = _vel_vgtu_vvssl(vrpin_r0s2, 0, 0, vl) ;

	__vr vrpin_r1s0 = _vel_vsfa_vvssl(vrij, 2, (uint64_t)(pInChannel+1*inWidth+0), vl) ;
	__vr vrin_r1s0 = _vel_vgtu_vvssl(vrpin_r1s0, 0, 0, vl) ;
	__vr vrpin_r1s1 = _vel_vsfa_vvssl(vrij, 2, (uint64_t)(pInChannel+1*inWidth+1), vl) ;
	__vr vrin_r1s1 = _vel_vgtu_vvssl(vrpin_r1s1, 0, 0, vl) ;
	__vr vrpin_r1s2 = _vel_vsfa_vvssl(vrij, 2, (uint64_t)(pInChannel+1*inWidth+2), vl) ;
	__vr vrin_r1s2 = _vel_vgtu_vvssl(vrpin_r1s2, 0, 0, vl) ;

	__vr vrpin_r2s0 = _vel_vsfa_vvssl(vrij, 2, (uint64_t)(pInChannel+2*inWidth+0), vl) ;
	__vr vrin_r2s0 = _vel_vgtu_vvssl(vrpin_r2s0, 0, 0, vl) ;
	__vr vrpin_r2s1 = _vel_vsfa_vvssl(vrij, 2, (uint64_t)(pInChannel+2*inWidth+1), vl) ;
	__vr vrin_r2s1 = _vel_vgtu_vvssl(vrpin_r2s1, 0, 0, vl) ;
	__vr vrpin_r2s2 = _vel_vsfa_vvssl(vrij, 2, (uint64_t)(pInChannel+2*inWidth+2), vl) ;
