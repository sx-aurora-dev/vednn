...
#undef INIT_VRSUM
  ...
  for (int64_t n=0; n<batch; n++) {
    for (int64_t y=0; y<gOutHeight; y+=nY) {
      ...
      __vr vrpin = _vel_vsfa_vvssl(vrij,2,(int64_t)(pInChannel+(y*strideHeight+r)*inWidth+s), vl) ;
      __vr vrin_r0s0 = _vel_vgtu_vvssl(vrpin, 0, 0, vl) ;

#define VLOAD_VRIN(R, S, RS)			\
  __vr vrin_##RS ;				\
  if( (R_UNROLL > R) && (S_UNROLL > S) ) {	\
    vrin_##RS = _vel_vgtu_vvssl(_vel_vaddsl_vsvl(4*(R*inWidth+S), vrpin, vl), 0, 0, vl) ;	\
  }						\
