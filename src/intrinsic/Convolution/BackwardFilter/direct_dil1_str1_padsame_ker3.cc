...
#pragma clang loop unroll(full)
	...
	for (int64_t x = 0; x < gOutWidth ; x += VLEN ) {
	  ...
	  __vr vrw_s0 = _vel_vaddsl_vsvl(0+x-1,  _vel_vseq_vl(vl), vl) ;
	  __vr vrw_s1 = _vel_vaddsl_vsvl(1+x-1,  _vel_vseq_vl(vl), vl) ;
	  __vr vrw_s2 = _vel_vaddsl_vsvl(2+x-1,  _vel_vseq_vl(vl), vl) ;

	  __vm256 vmw_s0 =  _vel_vfmklge_mvl(vrw_s0, vl) ;					// condition(0 <= w)
	  __vm256 vmw_s2 =  _vel_vfmklgt_mvl(_vel_vcmpsl_vsvl(inWidth,vrw_s2, vl), vl) ;	// condition(w < inWidth)

	  {
	    ...
	    __vr vrin_r1s0 = _vel_vldu_vssl(4,&pInChannel[(y  )*inWidth+x+0-1], vl) ;
	    __vr vrin_r1s1 = _vel_vldu_vssl(4,&pInChannel[(y  )*inWidth+x+1-1], vl) ;
	    __vr vrin_r1s2 = _vel_vldu_vssl(4,&pInChannel[(y  )*inWidth+x+2-1], vl) ;
	    __vr vrin_r2s0 = _vel_vldu_vssl(4,&pInChannel[(y+1)*inWidth+x+0-1], vl) ;
	    __vr vrin_r2s1 = _vel_vldu_vssl(4,&pInChannel[(y+1)*inWidth+x+1-1], vl) ;
	    __vr vrin_r2s2 = _vel_vldu_vssl(4,&pInChannel[(y+1)*inWidth+x+2-1], vl) ;
...
#define VFADD(VRIN,VRSUM0,VRSUM)								\
	    ...
	    vrin_r1s0 = _vel_vmrg_vsvml(0.f, vrin_r1s0, vmw_s0, vl) ;
	    VFADD(vrin_r1s0, vrsum0_r1s0, vrsum_r1s0) ;

	    VFADD(vrin_r1s1, vrsum0_r1s1, vrsum_r1s1) ;

	    vrin_r1s2 = _vel_vmrg_vsvml(0.f, vrin_r1s2, vmw_s2, vl) ;
	    VFADD(vrin_r1s2, vrsum0_r1s2, vrsum_r1s2) ;

	    vrin_r2s0 = _vel_vmrg_vsvml(0.f, vrin_r2s0, vmw_s0, vl) ;
	    VFADD(vrin_r2s0, vrsum0_r2s0, vrsum_r2s0) ;

	    VFADD(vrin_r2s1, vrsum0_r2s1, vrsum_r2s1) ;

	    vrin_r2s2 = _vel_vmrg_vsvml(0.f, vrin_r2s2, vmw_s2, vl) ;
	    VFADD(vrin_r2s2, vrsum0_r2s2, vrsum_r2s2) ;

	  } // gOutHeight
	  for (int64_t y = 1; y < gOutHeight-1 ; y ++ ) {

	    const float *pInChannel = pIn + inGroupOffset + ((n * inChannel + c) * inHeight * inWidth ) ;

	    const int64_t outIndex = gOutGroupOffset + ((n * gOutChannel + k) * gOutHeight + y) * gOutWidth + x  ;

	    __vr vrin_r0s0 = _vel_vldu_vssl(4,&pInChannel[(y-1)*inWidth+x+0-1], vl) ;
	    __vr vrin_r0s1 = _vel_vldu_vssl(4,&pInChannel[(y-1)*inWidth+x+1-1], vl) ;
	    __vr vrin_r0s2 = _vel_vldu_vssl(4,&pInChannel[(y-1)*inWidth+x+2-1], vl) ;
	    __vr vrin_r1s0 = _vel_vldu_vssl(4,&pInChannel[(y  )*inWidth+x+0-1], vl) ;
	    __vr vrin_r1s1 = _vel_vldu_vssl(4,&pInChannel[(y  )*inWidth+x+1-1], vl) ;
	    __vr vrin_r1s2 = _vel_vldu_vssl(4,&pInChannel[(y  )*inWidth+x+2-1], vl) ;
	    __vr vrin_r2s0 = _vel_vldu_vssl(4,&pInChannel[(y+1)*inWidth+x+0-1], vl) ;
...
#pragma clang loop unroll(full)
	    ...
	    vrin_r0s0 = _vel_vmrg_vsvml(0.f, vrin_r0s0, vmw_s0, vl) ;
	    VFADD(vrin_r0s0, vrsum0_r0s0, vrsum_r0s0) ;

	    VFADD(vrin_r0s1, vrsum0_r0s1, vrsum_r0s1) ;

	    vrin_r0s2 = _vel_vmrg_vsvml(0.f, vrin_r0s2, vmw_s2, vl) ;
...
#pragma clang loop unroll(full)
      for(int64_t kk=0; kk<nPacked; kk++) {
	__vr vrsumU_r0s0 = _vel_vfsums_vvl(vrsum_r0s0[kk], VLEN) ;
	_vel_vstu_vssl(vrsumU_r0s0, 4, pGKernel + FILTER_OFFSET(k+2*kk+remain,  c,0,0), 1) ;
	__vr vrsumL_r0s0 = _vel_vfsums_vvl(_vel_vsll_vvsl(vrsum_r0s0[kk],32, VLEN), VLEN);
	...
	__vr vrsumU_r0s2 = _vel_vfsums_vvl(vrsum_r0s2[kk], VLEN) ;
	_vel_vstu_vssl(vrsumU_r0s2, 4, pGKernel + FILTER_OFFSET(k+2*kk+remain,  c,0,2), 1) ;
	__vr vrsumL_r0s2 = _vel_vfsums_vvl(_vel_vsll_vvsl(vrsum_r0s2[kk],32, VLEN), VLEN);
	_vel_vstu_vssl(vrsumL_r0s2, 4, pGKernel + FILTER_OFFSET(k+2*kk+remain+1,c,0,2), 1) ;
	__vr vrsumU_r1s0 = _vel_vfsums_vvl(vrsum_r1s0[kk], VLEN) ;
	_vel_vstu_vssl(vrsumU_r1s0, 4, pGKernel + FILTER_OFFSET(k+2*kk+remain,  c,1,0), 1) ;
	__vr vrsumL_r1s0 = _vel_vfsums_vvl(_vel_vsll_vvsl(vrsum_r1s0[kk],32, VLEN), VLEN);
	...
	__vr vrsumU_r1s2 = _vel_vfsums_vvl(vrsum_r1s2[kk], VLEN) ;
	_vel_vstu_vssl(vrsumU_r1s2, 4, pGKernel + FILTER_OFFSET(k+2*kk+remain,  c,1,2), 1) ;
	__vr vrsumL_r1s2 = _vel_vfsums_vvl(_vel_vsll_vvsl(vrsum_r1s2[kk],32, VLEN), VLEN);
	_vel_vstu_vssl(vrsumL_r1s2, 4, pGKernel + FILTER_OFFSET(k+2*kk+remain+1,c,1,2), 1) ;
	__vr vrsumU_r2s0 = _vel_vfsums_vvl(vrsum_r2s0[kk], VLEN) ;
	_vel_vstu_vssl(vrsumU_r2s0, 4, pGKernel + FILTER_OFFSET(k+2*kk+remain,  c,2,0), 1) ;
	__vr vrsumL_r2s0 = _vel_vfsums_vvl(_vel_vsll_vvsl(vrsum_r2s0[kk],32, VLEN), VLEN);
	...
	__vr vrsumU_r2s2 = _vel_vfsums_vvl(vrsum_r2s2[kk], VLEN) ;
	_vel_vstu_vssl(vrsumU_r2s2, 4, pGKernel + FILTER_OFFSET(k+2*kk+remain,  c,2,2), 1) ;
	__vr vrsumL_r2s2 = _vel_vfsums_vvl(_vel_vsll_vvsl(vrsum_r2s2[kk],32, VLEN), VLEN);
