...
{
  std::ostringstream oss; // tmp buffer for OSSFMT etc.
  string const impl = "cjitConvFwd4";
  int const verbose=0;
...
#define DEF(VAR) def(#VAR, VAR)
  ...
  if( k<outChannelGroup ){
    ...
    if( k<kMax ){
      ...
      loop_r[".."]>>"#pragma clang loop unroll(disable)"; // clang possible segfault?
      auto& good_h=loop_r["good_h"];
      good_h>>"__vr vrw = vrj;";
      int64_t s=0, have_sBy=0;
      // NEXT choose when to do the "odd one out"s so that fast pKernValue alignment can be 8 ??
      if((kernWidth & 0x01) != 0){
        ...
        loop_c[".."]>>"#pragma clang unroll(disable)";
        loop_c
      ...
      if(s < kernWidth){
        ...
        have_sBy = sBy;
        assert( sBy==2 );
        loop_x0>>"__vr vrPsum = vzeros; /* in principle just vrsum, possible but :( */";
        CBLOCK_SCOPE(loop_sBy,"for (int64_t s="+asDec(s)+"; s<kernWidth; s+=sBy)",pr,/*loop_r*/good_h);
        loop_sBy.def("sBy",sBy);
          //>>"vrsum = _vel_vshf_vvvsl(vrsum,vzeros,VE_VSHUFFLE_YUZL, vl); /*nilpotent*/"
        loop_sBy[".."]>>"#pragma clang loop unroll(disable)"; // clang possible segfault?
        loop_sBy // for s: 0<=w and w<=inWidth; s+1 adds dilationWidth to w thresholds
          ...
          >>"vrw = _vel_vaddsl_vsvl(dilationWidth,  vrw2, vl);"
          ;
#if 1
        CBLOCK_SCOPE(loop_c_sBy,"for (int64_t c = 0; c < inChannelGroup; ++c)",pr,loop_sBy);
        loop_c_sBy[".."]>>"#pragma clang loop unroll(disable)";
        loop_c_sBy
          >>"const float *pIn = pIn_0 + c*inHW + (i+r*dilationHeight)*inWidth"
          >>"                 + x0*strideWidth-padWidth + s*dilationWidth;"
          >>"const float *pKerValue = pKern_gk + c*kernHW + r*kernWidth +s;"
          ...
          //>>"/*P*/ const uint64_t kerValue01 = *(uint64_t*)pKerValue; /*alignment might be wrong*/"
          >>"/*P*/ vrPsum = _vel_pvfmad_vvsvMvl(vrPsum, kerValue01, vrinP, vmP, vrPsum, vl);"
          ;
#else // block for loading of kernvalues (TESTING)
#endif
      }
      ...
      if(have_sBy){ // add the packed and unpacked sums
        x0_end
  ...
  if( k<outChannelGroup ){
    ...
    if( k<kMax ){
      ...
      bool const use_maskW = true; // original
      //bool const use_maskW = (padWidth==0 && strideWidth==1); // catch simplest case
      loop_s>>OSSFMT("// use_maskW="<<use_maskW<<" padWidth="<<padWidth<<" strideWidht="<<strideWidth);
      if(use_maskW){
      ...
      loop_s[".."]>>"#pragma clang loop unroll(disable)";

      // c=0..inChannelGroup blocking
      auto& x0_end = loop_x0["induce+write"];
      int have_cBy=0;

      // unroll starting from ANY c<inChannelGroup, in groups of cBy,
      auto do_cBy = [](int& c, int const& cBy, int& cByMax, int const& inChannelGroup,
          Cblock &loop_x0, bool const use_maskW)
      {
        Cunit& pr = loop_x0.getRoot();
        Cblock& loop_s = loop_x0.at("**/loop_s/body");
        std::ostringstream oss;
        int const c0 = c;
        printf("do_cBy c=%d to inChannelGroup=%d by %d",(int)c0,(int)inChannelGroup,(int)cBy);
        if( cBy <= 0 || c+cBy > inChannelGroup){
          printf(" (nop)\n");
          return;
        }
        int const c_final = c0 + (inChannelGroup-c0)/cBy*cBy;
        printf(" --> final c= %d\n",(int)c_final);
        CBLOCK_SCOPE(scBy,"",pr,loop_s);
        scBy>>OSSFMT("/* int const cBy="<<cBy<<"; */");
        CBLOCK_SCOPE(loop_c,OSSFMT("for (int64_t c="<<c<<"; c<"<<c_final<<"; c+="<<cBy<<")"),pr,scBy);
        loop_c[".."]>>"#pragma clang unroll(disable)";
#if 1 // at what 'c' does loop_c exit? [DEBUG paranoia]
        for ( ; c+cBy <= inChannelGroup; c+=cBy) /*printf(" c=%d\n",(int)c)*/; /*NOP*/
        printf(" final c = %d", (int)c);
        assert( c == c_final );
#endif
        printf("\n");
        loop_c
          ...
          >>"const float *pKerValue = pKern_gk + c*kernHW + r*kernWidth +s;"
          ;
        // _vel_pack_f32p could use some precalc (re-order the kernel weights), because it is ldu/ldl.zx/or
        char const* summation_op = (use_maskW
            ?"\nvrsum_CBY = _vel_pvfmad_vvsvMvl(vrsum_CBY, ker_CBY, vrin_CBY, vmP, vrsum_CBY, vl);"
            :"\nvrsum_CBY = _vel_pvfmad_vvsvl(vrsum_CBY, ker_CBY, vrin_CBY, vl);");
        // for preread, can split the ops:
        static string update_vrin_ker_CBY =
          "\n/*cByCBY*/ __vr vrin_CBY = _vel_vldu_vssl(4*strideWidth,pIn+(CBY-1, vl)*inHW);"
          "\nconst uint64_t ker_CBY = _vel_pack_f32p("
          "\n                                pKerValue+(CBY-1)*kernHW,"
          "\n                                pKerValue+(CBY-1)*kernHW + inChannelGroup*kernHW);" ;
        static string shuff_sum_CBY = OSSFMT(
          "\nvrin_CBY = _vel_vshf_vvvsl(vrin_CBY, vrin_CBY, VE_VSHUFFLE_YUZU, vl);"
          <<summation_op);
        static string zero_vrsum_CBY =
          "__vr vrsum_CBY = vzeros;";
        // limit summing registers to some lower value?
        for(int ccBy=1; ccBy<=cBy; ++ccBy){
          if(ccBy > 1 && ccBy >= cByMax){
            loop_x0>>multiReplace("CBY",OSSFMT(ccBy), zero_vrsum_CBY);
          }
        }
#if 0 // simplest, seems pretty good.
        // or all in one:
        static string update_vrsum_CBY = OSSFMT(update_vrin_ker_CBY<<shuff_sum_CBY);
        for(int ccBy=1; ccBy<=cBy; ++ccBy){
          string cBy_numeric=asDec(ccBy);
          loop_c >>multiReplace("CBY",cBy_numeric, update_vrsum_CBY);
        }
#else // 'pre'-load some data from memory, then start up the compute loop
        {
          int pre = 2; // move loads up by... 2 (better, but within noise?)
          int ccBy0 = 1;
          string strccBy0, strccBy1;
          int ccBy1;
          for(int ccBy=ccBy0; ccBy <= cBy && ccBy < ccBy0+pre; ++ccBy){
              loop_c >> multiReplace("CBY",OSSFMT(ccBy), update_vrin_ker_CBY);
          }
          loop_c>>"/*end preload pre="+asDec(pre)+" */";
          for(; ccBy0<=cBy; ++ccBy0){
            ccBy1 = ccBy0 + pre;
            if( ccBy1 <= cBy ){       // preread...
              loop_c>>multiReplace("CBY",OSSFMT(ccBy1), update_vrin_ker_CBY);
            }
            loop_c >>multiReplace("CBY",OSSFMT(ccBy0), shuff_sum_CBY); // shuff_sum
          }
        }
        // XXX Can we limit the number of pvfmad registers (less to coadd later) ?
        // e.g. 4 summing registers even for unroll of 8 or greater ?
        // Ex. multireplace("CBY",OSSFMT(ccBy%4+1), ...)
#endif
        if( cBy > cByMax ) cByMax = cBy; // ?
      };
#if 1
      auto sum_cBy = [](int const& cByMax, Cblock& x0_end){
        // optimize the multiple terminal _vel_pvfadd_vvv summations l(binary-tree-like add, vl)
        {
          ...
          for( ; d<cByMax; d=d2 ){
            ...
            for( ; i<cByMax; i+=d2 ){
              if(i+d<cByMax){
                vrA = "vrsum_"+asDec(i+1);
                vrB = "vrsum_"+asDec(i+d+1);
...
#else // more useful API
      // pvfadd many regs into regs.front()
      void ve_pvfadd(Cblock& cb, std::unordered_set<std::string> const& regs){
        size_t const nregs = regs.size();
        // copy into direct-access container
        std::vector<std::string> vregs;
        vregs.reserve(nregs);
        for(auto& vr: regs) vregs.push_back(vr);
        ve_pvfadd(cb, vregs);
      }
      void ve_pvfadd(Cblock& cb, std::vector<std::string> const& regs){
        std::ostringstream oss;
        unsigned d=1, d2;
        cb>>"// sum_pack_floats";
        ...
        for( ; d<cByMax; d=d2 ){
          d2 = d<<1;
          unsigned i=0;
          for(unsigned i=0; i<cByMax; i+=d2 ){
            if(i+d<cByMax){
              oss<<vregs[i]" = _vel_pvfadd_vvvl("<<regs[i]<<", "<<regs[i+d]<<", vl);\n"
            }
          }
        }
      };// end multiple pvfadd
...
#if 1 // NEW generalization
      int c = 0, cByMax = 0;
      if( inChannelGroup <= 16 ) have_cBy == inChannelGroup;
      //else{
      //  for(int ccBy=15; ccBy>=8; --ccBy){
      ...
      //  }
      //}
      if(!have_cBy) have_cBy = 8; // 8-z-12770575 4-z-13257492
      //
      do_cBy(c, have_cBy, cByMax, inChannelGroup, loop_x0, use_maskW);
      // now all remaining (unrolled)
      have_cBy = inChannelGroup - c;
      do_cBy(c, have_cBy, cByMax, inChannelGroup, loop_x0, use_maskW);
      // afterward, co-add the separate summing registers
      sum_cBy( cByMax, x0_end );
#else
      int c = 0;
      for(int ccBy=11; ccBy>4; --ccBy){
        if( inChannelGroup%ccBy == 0 ){
          have_cBy = ccBy;
          break;
        }
  ...
  if( k<outChannelGroup ){
    ...
    if( k<kMax ){
      ...
      CBLOCK_SCOPE(loop_s,"for (int64_t s = 0; s < kernWidth; ++s)",pr,loop_r);
      loop_s
        >>"__vm256 vm2 = _vel_vfmkl_mcvl(VECC_GE, vrw, vl); // condition(0 <= w)"
        //>>"__vm256 vm3 = _vel_vfmkl_mcvl(VECC_IG, _ve_vcmpsl_vsv(inWidth,vrw, vl)); // condition(w < inWidth)"
        //>>"__vm256 vm23  = _vel_andm_mmml(vm2, vm3, vl);"
        >>"__vm256 vm23 = _vel_vfmkl_mcvml(VECC_IG, _ve_vcmpsl_vsv(inWidth,vrw, vl),vm2); // && condition(w < inWidth)"
        ;
      loop_s[".."]>>"#pragma clang loop unroll(disable)"; // clang possible segfault?
      CBLOCK_SCOPE(loop_c,"for (int64_t c = 0; c < inChannelGroup; ++c)",pr,loop_s);
    loop_c[".."]>>"#pragma clang loop unroll(disable)"; // clang possible segfault?
      loop_c
        >>"const float *pIn = pIn_0 + c*inHW + (i+r*dilationHeight)*inWidth"
        >>"                 + x0*strideWidth-padWidth + s*dilationWidth;"
        >>"__vr vrin = _vel_vldu_vssl(4*strideWidth,pIn, vl);"
